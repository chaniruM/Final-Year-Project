...
/// Processes biometric signals over time to determine drowsiness and distraction levels.
///
/// Uses a combination of raw thresholds and temporal analysis (e.g., PERCLOS)
/// to generate an overall alertness score.
class FusionEngine {
  ...
  /// Evaluates the current frame's biometric data and updates the internal state.
  ///
  /// Returns a map containing the calculated alert level, status message, score,
  /// and smoothed signal values.
  Map<String, dynamic> processFrame({
    ...
    // Ignore -1.0 values (occlusions) to prevent breaking the smoothing buffer.
    ...
    bool isNoddingInstant =
        relativePitch < DrowsinessConstants.headNodPitchThreshold;

    // Occlusion Detection (Focus strictly on Eyes/Sunglasses)
    if (currentEar >= 0.0) {
      _lastEyesDetectedTime = now;
    }

    final bool isOccluded =
        now.difference(_lastEyesDetectedTime ?? now).inSeconds > 2;

    // Update History & PERCLOS
    ...
    if (!isOccluded) {
      ...
      _timeBuffer.removeWhere((frame) =>
          now.difference(frame.timestamp) >
          DrowsinessConstants.perclosWindowDuration);

      // Microsleep Detection
      if (eyesClosed && !isYawningInstant) {
        _blinkStartTime ??= now;
        if (now.difference(_blinkStartTime!).inMilliseconds >
            DrowsinessConstants.microsleepDurationMs) {
          isMicrosleep = true;
        }
      } else {
        _blinkStartTime = null;
    ...
    double effectivePerclosThreshold =
        _baselinePerclos + DrowsinessConstants.perclosTolerance;
    ...
    if (isNoddingInstant) {
      _droopStartTime ??= now;
      if (now.difference(_droopStartTime!).inMilliseconds >
          DrowsinessConstants.headDroopDurationMs) {
    ...
    if (isYawningInstant) {
      _yawnStartTime ??= now;
      if (now.difference(_yawnStartTime!).inMilliseconds >
          DrowsinessConstants.yawnDurationMs) {
        isContinuousYawn = true;
      }
    } else {
      if (_yawnStartTime != null &&
          now.difference(_yawnStartTime!).inMilliseconds >
              DrowsinessConstants.yawnDurationMs) {
    ...
    // DYNAMIC FUSION SCORING (Multi-Model Occlusion Mode)
    if (isOccluded) {
    ...
    // Determine Status & Alert Level
    int alertLevel = 0;
    String status = "Monitoring (Score: ${score.toInt()})";

    if (isMicrosleep ||
        isContinuousDroop ||
        score >= DrowsinessConstants.scoreThresholdAlert) {
      alertLevel = 2;
      if (isMicrosleep) {
      ...
      } else if (_verifiedYawns.length >=
          DrowsinessConstants.frequentYawnCount) {
        status = "ALERT: EXTREME FATIGUE (Frequent Yawns)";
      } else {
        status = "ALERT: Drowsiness Detected!";
      }
    } else if (isContinuousYawn ||
        score >= DrowsinessConstants.scoreThresholdWarning) {
      alertLevel = 1;
      if (isContinuousYawn) {
      ...
      // Provide low-risk status updates even during occlusion scenarios.
      if (isYawningInstant) {
